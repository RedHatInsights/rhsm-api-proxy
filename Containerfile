...
RUN curl -sfL --retry 10 -o /tmp/go.tar.gz https://go.dev/dl/go1.22.2.linux-amd64.tar.gz && \
    echo "5901c52b7a78002aeff14a21f93e0f064f74ce1360fce51c6ee68cd471216a17 /tmp/go.tar.gz" | sha256sum -c && \
    mkdir -p $GODIR && \
